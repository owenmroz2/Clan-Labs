...
bot.on('message', async message => {
  ...
  if (message.content.toLowerCase().startsWith(`${config.prefix}${config.xpName}`)){
    ...
    if (!args[1]){
      return message.channel.send(`Sorry ${message.author}, but you're missing the first argument--add or remove?\n**Adding XP: \`${config.prefix}${config.xpName} add 1 username1, username2, username3...\`\nRemoving XP: \`${config.prefix}${config.xpName} remove 1 username1, username2, username3...\`**`).then(message => message.delete(5000));
  ...
  if (message.content.toLowerCase().startsWith(`${config.prefix}commands`)){
    ...
    var second = new Discord.RichEmbed()
      ...
      .addField(`**\`${config.prefix}${config.xpName} add 1 username1, username2, username3, etc\`**`, `Adds 1 XP to the usernames provided (\`username1, username2, username3, etc\`).`)
      .addField(`**\`${config.prefix}${config.xpName} remove 1 username1, username2, username3, etc\`**`, `Removes 1 XP to the usernames provided (\`username1, username2, username3, etc\`).`)
